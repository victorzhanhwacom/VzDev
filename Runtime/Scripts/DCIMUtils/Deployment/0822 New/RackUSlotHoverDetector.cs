...
namespace VzDev.DCIMUtils.DeploymentUtils
{
    ...
    public class RackUSlotHoverDetector : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (instanceRef != null)
            {
                Debug.LogError($"{GetType().Name} 場景上重複存在，此 instance 將被銷毀：{gameObject.name}", this);
                isDuplicate = true;
                Destroy(gameObject);
                return;
            }
        ...
        {
            if (isDuplicate) return;
            if (selectedRackAsset == null) return;
            ...
            if(hit.collider.transform.parent != selectedRackAsset.modelInfo.modelTarget)
            {
                ClearCurrent();
                return;
            }

            if (hit.collider.TryGetComponentInParent(out DataModelBinder_Rack rackDataBinder) == false)
            {
                ClearCurrent();
                return;
            }

            int uIndex = CalculateUIndex(hit.collider, hit.point, rackDataBinder.RackAsset);


            /// 只有當機櫃或 U 槽編號改變時才廣播，避免每幀都廣播造成效能浪費
            if (rackDataBinder.RackAsset == currentRackAsset && uIndex == currentUIndex) return;
            currentRackAsset = rackDataBinder.RackAsset;
        ...
        public DCR_Asset selectedRackAsset;

        private void HandleSelectRackTarget(DCR_Asset asset)
        {
            selectedRackAsset = asset;
        }
        private void HandleDeselectRackTarget()
        {
        ...
        {
            DeployToRackSelector.OnSelectRackTargetAction += HandleSelectRackTarget;
            DeployToRackSelector.OnDeselectRackTargetAction += HandleDeselectRackTarget;
        }
